---
title: "Final_All_Code_Student_Performance"
subtitle: "Group L02G06"
author: "Niharika Chomal, Yutong Han, Zichun Han, Yizhen Pan and Hannah Pwint Shwe Yi Oo"
output:
  html_document:
    code_folding: hide
---
...
```
Disclaimer: <br>
  - Dataset description/ EDA investigation code is written by Hannah Pwint Shwe Yi Oo <br>
  - Model Selection Code is written by Yizhen Pan. <br>
  - Assumption Code is written by Zichun Han. <br>

NOTE: Data Used in this file should be in a folder called 'student' (the student.zip file given to us) in the same level directory as this file. 
...
Dataset description/ EDA investigation code is written by Hannah Pwint Shwe Yi Oo
...
#### Code for EDA of G3 for the report
```{r}
...
my.og.d1 <- my.og.d1 %>% dplyr::select(Subject, G3)
my.og.d2 <- my.og.d2 %>% dplyr::select(Subject, G3)
...
barplot / boxplot + plot_layout(heights = c(2, 1)) 
```
...
```{r}
log.d1 %>% dplyr::select(nursery, Fjob, G3) %>% pivot_longer(-G3) %>% 
...
```{r}
log.d2 %>% dplyr::select(nursery, Fjob, G3) %>% pivot_longer(-G3) %>% 
  ggplot(aes(x = value, y = G3, fill = value)) +
  geom_boxplot(color = "black", alpha = 0.7) +
  facet_wrap(~name, scales = 'free') +
...
## More Investigation for the trends between G3 and variables
...
selected.log.d1 <- log.d1 %>% 
  dplyr::select(address, Medu, Mjob, traveltime, nursery, age, Dalc, failures, Fedu, freetime, Medu, studytime, traveltime, Walc, G3)
...
```{r}
log.d2 %>% dplyr::select(where(is.numeric)) %>% pivot_longer(-G3) %>% 
  ggplot(aes(x=value,y=G3))+geom_jitter(width=0.1,height=1,alpha=0.2)+
  facet_wrap(~name,scales='free') + 
  labs(x = "Independent Features", y = "Log G3", title = "Portuguese Data Relationship with Each Numerical Feature")
```
...
Model Selection Code is written by Yizhen Pan. 
...
## Assumption checking
Assumption Code is written by Zichun Han.
...
```{r}
port_data %>%
  dplyr::select(G2, G1, traveltime, Medu, age, absences, failures, G3) %>%
  pivot_longer(-G3) %>%
  ggplot(aes(x = value, y = G3)) +
  geom_jitter(width = 0.1, height = 1, alpha = 0.2) +
  geom_smooth(method = "lm", color = "red", se = FALSE) +
...
```{r}
math_data %>%
  dplyr::select(G2, famrel, G1, failures, health, G3) %>%
  pivot_longer(-G3) %>%
  ggplot(aes(x = value, y = G3)) +
  geom_jitter(width = 0.1, height = 1, alpha = 0.2) +
  geom_smooth(method = "lm", color = "red", se = FALSE) +
